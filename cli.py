"""handbrake CLI — static gate for autonomous agents with no human-in-the-loop.

Flags a destructive/irreversible tool (delete, payment, send, deploy, shell,
file overwrite, external POST/PUT/DELETE) that is exposed to an autonomous LLM
agent with no human-approval, confirmation, or human-in-the-loop gate. The core
is AST-based and never imports or runs the target code, so it is safe to run
over untrusted source in CI.
"""
...
_IGNORE_FILE = ".handbrakeignore"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="handbrake",
        description="Static gate for autonomous agents wired to destructive "
        "tools with no human-in-the-loop. Flags a destructive/irreversible "
        "tool exposed to an autonomous agent with no approval gate (HB001) and "
        "a destructive agent tool whose wiring/approval is uncertain (HB002). "
        "AST-based; no target code is ever executed.",
    )
    parser.add_argument(
        "paths", nargs="*",
    ...
    parser.add_argument("--strict", action="store_true",
                        help="Also fail (exit 1) on warnings (HB002), not just blockers.")
    parser.add_argument("--json", action="store_true", dest="as_json",
                        help="Output machine-readable JSON.")
    parser.add_argument("--no-color", action="store_true", help="Disable colored output.")
    parser.add_argument("--exclude", action="append", default=[], metavar="GLOB",
                        help="Exclude paths matching GLOB (repeatable).")
    parser.add_argument("--version", action="version", version=f"handbrake {__version__}")
    return parser


def _load_ignore_patterns(root: str) -> List[str]:
    """Read glob patterns from a .handbrakeignore in the current directory."""
